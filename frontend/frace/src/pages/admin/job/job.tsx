...
import dayjs from 'dayjs';
import { callDeleteJob, callFetchJobsByCompany } from "@/config/api";
import queryString from 'query-string';
import { useNavigate } from "react-router-dom";
import { fetchJob } from "@/redux/slice/jobSlide";
import Access from "@/components/share/access";
...
const JobPage = () => {
    ...
    const isFetchingJos = useAppSelector(state => state.job.isFetching);
    const meta = useAppSelector(state => state.job.meta);
    const jobs = useAppSelector(state => state.job.result);
    const [jobsByCompany, setJobsByCompany] = useState<IJob[]>();
    ...
    const fetchJobsByCompany = async () => {
        setIsFetching(true);
        const res = await callFetchJobsByCompany();
        if (res && res.data) {
            setJobsByCompany(res.data.result);
        }
        setIsFetching(false);
    }
        ...
        <div>
            ...
            >
                <DataTable<IJob>
                    ...
                    headerTitle="Danh sách Jobs"
                    rowKey="id"
                    loading={isAdmin ? isFetchingJos : isFetching}
                    columns={columns}
                    dataSource={isAdmin ? jobs : jobsByCompany}
                    request={isAdmin
                        ...
                        : async (): Promise<any> => {
                            dispatch(callFetchJobsByCompany)
                        }}
